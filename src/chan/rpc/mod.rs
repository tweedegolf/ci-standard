...
#[derive(Clone)]
/// A channel for publishing messages on direct queues, allowing for receiving
/// replies using [RpcBus]. It also supports publishing an initial message on a
/// direct queue, and setting up a back-and-forth communincation channel using
/// [RpcCommBus].
...
impl RpcChannel {
    ...
    /// with this [RpcChannel]. Any incoming replies are forwarded to the
    /// [Future]s that correspond the the message correlation [Uuid].
        ...
        let handle_replies: JoinHandle<Result<()>> = task::spawn({
            ...
            async move {
                while let Some(msg_res) = reply_consumer.next().await {
                    match msg_res {
                        Ok(msg) => {
                            // Spawn a task that attempts to forward the reply `msg` that just came
                            // in Getting a lock to pending_replies may
                            // block
                            let forward_reply: JoinHandle<()> = task::spawn_blocking({
                                ...
                                move || {
                                    let reply_id = match delivery_uuid(&msg, 1) {
                                        ...
                                        Some(Err(e)) => {
                                            error!(
                                                "Error parsing reply message correlation UUID: \
                                                 {e:?}. Dropping message."
                                            );
                                            return;
                                        }
                                        None => {
                                            error!(
                                                "Received reply with nog correlation ID. Dropping \
                                                 message."
                                            );
                                            return;
                                        }
                                    ...
                                    if !forwarding_success {
                                        warn!(
                                            "Received reply cannot be forwarded due to dropped \
                                             Receiver. UUID: {}",
                                            reply_id
                                        );
                                    }
...
impl Channel for RpcChannel {
    async fn publish_with_properties(
        ...
        debug!(
            "Publishing message with correlation ID {correlation_id} an RPC channel with routing \
             key {routing_key}"
        );
    ...
    B::ReplyPayload: Deserialize<'r> + Serialize,
{
    ...
    pub async fn publish_recv_many(
        ...
        debug!(
            "Publishing message with correlation UUID {correlation_uuid}, expecting one or more \
             replies"
        );
...
mod tests {
    ...
    // use uuid::Uuid;
    use crate::{
        chan::tests::FramePayload,
        rpc_bus, /* setup_test_logging, Connection, Consumer, Publisher,
                 * RpcChannel, */
    };
    ...
    // Connection::connect(RABBIT_MQ_URL).await?;     let uuid =
    // Uuid::new_v4();     tokio::task::spawn({
    ...
    //             for _ in 0..3 {
    //                 msg.reply(&Err(FrameSendError::ClientDisconnected),
    // &channel)                     .await
    //                     .unwrap();
    //             }
    //         }
...
macro_rules! rpc_bus {
    (
        ...
        $crate::rpc_bus_impl!($bus, $reply_payload, $serialize, $deserialize);
    };
    (
        doc =
        $doc:literal,bus =
        $bus:ident,publish =
        $publish_payload:ty,reply =
        $reply_payload:ty,args =
        $args:ty,queue =
        $queue:expr,serialize =
        $serialize:expr,deserialize =
        $deserialize:expr
    ) => {
        $crate::rpc_bus!(
            $doc,
