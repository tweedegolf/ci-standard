...
pub trait RpcCommBus: Unpin {
    ...
    /// The type of payload of the messages that the receiver of the initial
    /// message sends back
    type BackPayload;
    /// The type of payload of the messages that the initiator of the
    /// communication sends forth
    type ForthPayload;
...
impl RpcChannel {
    ...
    pub async fn comm_consumer<B: RpcCommBus>(
        ...
        debug!(
            "Created consumer for RPC comm bus {} for queue {queue} with consumer tag \
             {consumer_tag}",
            type_name::<B>()
        );
...
{
    /// Publish an initial message onto the [RpcCommBus], and receive the
    /// replies with with [RpcCommBus::BackPayload] payloads onto the
    /// returned [Stream]. The replies can be obtained by calling
    /// [futures::StreamExt::next] on the returned [Stream].
    pub async fn publish_recv_comm(
        ...
        debug!(
            "Publishing message with correlation UUID {correlation_uuid}, setting up \
    ...
    B::ForthPayload: Deserialize<'f> + Serialize,
{
    ...
    /// and await multiple replies from the receiver.
    /// The messages can be obtained by calling [futures::StreamExt::next] on
    /// the returned [Stream].
    ...
    /// Reply to a message that was sent by the receiver of the initial message
    /// and await one reply from the receiver.
...
mod tests {
    ...
    use crate::{
        ...
        rpc_comm_bus,   // setup_test_logging, Connection, Consumer,
        FrameSendError, // Publisher, RpcChannel,
    };
    ...
    // response_fut).await.unwrap();                 info!("Got forth reply
    // {i}");                 assert_eq!(response.get_payload().unwrap(),
    // Ok(()))             }
    //         }
    //     });
...
macro_rules! rpc_comm_bus {
    ...
    (
        ...
        $doc:literal,bus =
        $bus:ident,initial =
        $initial_payload:ty,back =
        $back_payload:ty,forth =
        $forth_payload:ty,args =
        $args:ty,queue =
