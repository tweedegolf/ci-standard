...
impl<E: TopicExchange> TopicChannel<E> {
    ...
    pub async fn consumer<B: TopicBus>(
        ...
        let consumer = self
            ...
            .await?;
        debug!(
            "Created consumer for topic bus {} with routing key {routing_key} and consumer tag \
             {consumer_tag}",
            type_name::<B>()
        );
        Ok(Consumer {
...
impl<E: TopicExchange> Channel for TopicChannel<E> {
    async fn publish_with_properties(
        ...
        debug!(
            "Publishing message with correlation UUID {correlation_uuid} on Topic Exchange '{}' \
...
{
    /// Publish a message onto a topic on the exchange associated with the
    /// [TopicBus] for this [Publisher] with the passed [PublisherRoutingKey].
...
/// Can only represent concrete routing keys, i.e. routing keys cannot contain
/// wildcards.
#[derive(Debug)]
...
#[derive(Debug)]
/// Error indicating what went wrong in setting up a [ConsumerRoutingKey] or a
/// [PublisherRoutingKey]
pub enum RoutingKeyError {
    /// Got a routing key that does not match the topic pattern associated with
    /// the [TopicBus].
...
mod tests {
    ...
    use crate::{
        topic_bus,
        topic_exchange, // Connection, Consumer,
        ConsumerRoutingKey,
        FramePayload,
        // Publisher,
        PublisherRoutingKey,
    ...
    //     tokio::task::spawn({
    //         let channel: TopicChannel<MyExchange> =
    // TopicChannel::new(&connection).await.unwrap();         let mut consumer:
    // Consumer<MyTopic> = channel
    ...
    //     let channel: TopicChannel<MyExchange> =
    // TopicChannel::new(&connection).await.unwrap();     let publisher:
    // Publisher<MyTopic> = channel.publisher();
    ...
    // RabbitMQ accepts this, but it would result in consumption of all messages on the topic
    // exchange
    #[test_case("#"; "Hash 6")]
...
macro_rules! topic_bus {
    ...
    (
        ...
        $doc:literal,bus =
        $bus:ident,publish =
        $publish_payload:ty,exchange =
        $exchange:ty,topic =
        $topic:literal,serialize =
